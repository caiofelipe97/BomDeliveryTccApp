    ...
    <ListItem key={i} style={styles.optionStyle}>
      <View style={styles.boxStyle}>
        ...
        <Icon
          ...
          onPress={() => {
            handleMultipleChange(i, 'plus');
          }}
        />
        <Text style={styles.nameStyle}>{option.name} </Text>
      </View>
      {option.value && (
        <Text style={styles.valueStyle}>{formatMoney(option.value)}</Text>
      )}
      {option.additional && (
        <Text style={styles.valueStyle}>+{formatMoney(option.additional)}</Text>
      )}
    </ListItem>
  ));
  return (
    <View>
      ...
      <Text note style={styles.descriptionStyle}>
        {' '}
