...
const InProgressScreen = ({navigation}) => {
  ...
  const inProgressOrders = inProgressOrders => {
    return inProgressOrders.map((order, index) => (
...
InProgressScreen.navigationOptions = ({navigation}) => {
  return {
    headerTitle: 'Pedidos Em Andamento',
    headerLeft: (
      <Button transparent>
